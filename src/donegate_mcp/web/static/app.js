...
(() => {
  ...
  function renderOverview() {
    ...
    const attention = tasks.filter(task => task.needs_revalidation || task.evidence_stale || task.verification_status === "failed" || task.status === "blocked");
    return `<div class="metrics">${metric("交付完成度", percent(summary), `${number(summary.done_tasks)} / ${number(summary.total_tasks)} 个任务已完成`, "teal")}${metric("当前验证通过", number(summary.current_verified_tasks), "当前输入下仍然有效的验收证据", "", `/ ${number(summary.total_tasks)}`)}${metric("需要重新验证", number(summary.verification_attention_tasks), "独立于历史交付状态", "amber")}${metric("阻塞任务", number(summary.blocked_tasks), "查看阻塞原因与后续行动", "red")}</div>${renderBatches()}<div class="overview-columns"><section class="panel"><div class="section-heading"><h2>完成状态分布</h2><span>${number(summary.done_tasks)} / ${number(summary.total_tasks)} 已完成</span></div>${number(summary.total_tasks) ? `<div class="distribution">${statuses.map(status => `<div class="distribution-row"><span>${labels[status]}</span>${progress(counts[status], summary.total_tasks, `${labels[status]} ${number(counts[status])} 个`)}<strong>${number(counts[status])}</strong></div>`).join("")}</div>` : '<p class="muted">项目尚无任务。添加任务后，这里会显示实际状态分布。</p>'}</section><section class="panel"><div class="section-heading"><h2>需要关注</h2><a href="${projectURL(state.route.key,"features")}" data-nav>查看功能 →</a></div>${attention.length ? `<div class="attention-list">${attention.slice(0,6).map(task => `<div class="attention-item"><div><h3>${taskLink(id(task),task.title || id(task))}</h3><p>${escape(task.blocked_reason || task.stale_reason || (task.verification_status === "failed" ? "当前回归失败，请查看失败证据。" : task.evidence_stale ? "验证证据已过期，历史交付记录保留。" : "需求或输入已变化，请重新验证当前实现。"))}</p></div>${badge(task.status === "blocked" ? "blocked" : "revalidation",task.status === "blocked" ? undefined : task.verification_status === "failed" ? "回归失败" : task.evidence_stale ? "证据已过期" : "待重验")}</div>`).join("")}</div>${attention.length > 6 ? `<p class="muted">另有 ${attention.length - 6} 个任务需要关注。</p>` : ""}` : '<div class="empty-state"><div class="empty-symbol" aria-hidden="true">✓</div><h2>当前没有阻塞或验证提醒</h2><p>验证和需求变化会反映在这里。</p></div>'}</section></div><section class="panel"><div class="section-heading"><h2>工作空间信息</h2><span>登记项目的实际路径</span></div><dl class="project-paths"><dt>项目标识</dt><dd>${escape(project.project_id || "未提供")}</dd><dt>仓库目录</dt><dd>${escape(project.repo_root)}</dd><dt>数据目录</dt><dd>${escape(project.data_root || "项目默认目录")}</dd><dt>最近更新</dt><dd>${escape(date(state.detail.updated_at))}</dd></dl></section>`;
  }

  function renderBatches() {
    const batches = Array.isArray(state.detail.batches) ? state.detail.batches : [];
    const tasks = state.detail.tasks || [];
    const modes = {single: "单功能验证", batch: "批量统一回归", auto: "自动选择"};
    return `<section class="panel batch-panel"><div class="section-heading"><h2>开发批次</h2><span>${batches.length} 个批次</span></div>${batches.length ? batches.map(batch => {
      const members = tasks.filter(task => (batch.task_ids || []).includes(id(task)));
      const delivered = members.filter(task => task.status === "done").length;
      const healthy = members.filter(task => task.verification_status === "passed" && !task.evidence_stale && !task.needs_revalidation).length;
      const run = batch.latest_run;
      const key = detailKey("batch", batch.batch_id);
      return `<details class="batch-item" data-open-key="${escape(key)}"${openAttribute(key)}><summary data-focus="batch-${escape(batch.batch_id)}"><strong>${escape(batch.title || batch.batch_id)}</strong><span>${escape(modes[batch.selected_mode || batch.mode] || "待选择")}</span></summary><p class="muted">${escape(batch.batch_id)} · 已交付 ${delivered} / ${members.length} · 当前验证通过 ${healthy} / ${members.length}</p><p>${escape(batch.rationale || "按任务规模与风险选择验证方式")}</p>${run ? `<p class="batch-evidence">最近回归：执行 ${number(run.executed_count)} 项检查 · 复用 ${number(run.reused_count)} 项检查${run.ok === false ? " · 有未通过的功能" : ""}</p>` : '<p class="muted">尚未执行批次回归</p>'}<div class="batch-members">${members.map(task => `<span>${taskLink(id(task), task.title || id(task))}${badge(task.status)}</span>`).join("")}</div></details>`;
    }).join("") : '<p class="muted">尚未建立开发批次。相关功能可组成一个批次，共享回归证据，分别保留功能进度。</p>'}</section>`;
  }
