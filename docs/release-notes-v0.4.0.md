...
- Exposes advisory summaries in task payloads, dashboard, progress, and supervision, with spawned follow-ups separated from unresolved open advisories.
- Lists tasks with pending advisory reviews and emits a lightweight plugin stop reminder when advisory attention remains.
- Preserves requested and completed provider audit fields on review runs.
...
Repeated `submit` or `done` lifecycle commands reuse the existing advisory review request for that checkpoint instead of creating duplicate pending requests.
