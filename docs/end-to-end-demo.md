...
donegate --data-root /tmp/delivery-demo/.donegate-mcp init --project-name demo
```

## Create a task

```bash
donegate --data-root /tmp/delivery-demo/.donegate-mcp --json task create \
...
```bash
TASK_ID=TASK-0001
donegate --data-root /tmp/delivery-demo/.donegate-mcp task transition $TASK_ID --to ready
donegate --data-root /tmp/delivery-demo/.donegate-mcp task start $TASK_ID
donegate --data-root /tmp/delivery-demo/.donegate-mcp task submit $TASK_ID
donegate --data-root /tmp/delivery-demo/.donegate-mcp --json task self-test $TASK_ID --workdir /tmp/delivery-demo
donegate --data-root /tmp/delivery-demo/.donegate-mcp task doc-sync $TASK_ID --result synced --ref /tmp/delivery-demo/docs/plan.md
donegate --data-root /tmp/delivery-demo/.donegate-mcp --json task done $TASK_ID
# later, if new work invalidates the closure:
donegate --data-root /tmp/delivery-demo/.donegate-mcp --json task reopen $TASK_ID
```
...
```bash
printf 'version 2\n' >/tmp/delivery-demo/docs/spec.md
donegate --data-root /tmp/delivery-demo/.donegate-mcp --json spec refresh --spec-ref /tmp/delivery-demo/docs/spec.md --reason "design updated"
donegate --data-root /tmp/delivery-demo/.donegate-mcp --json progress
```
...
```bash
donegate --data-root /tmp/delivery-demo/.donegate-mcp deviation add $TASK_ID \
  --summary "temporary workaround" \
  --details "using compatibility path until final API lands"
donegate --data-root /tmp/delivery-demo/.donegate-mcp --json deviation list
```
